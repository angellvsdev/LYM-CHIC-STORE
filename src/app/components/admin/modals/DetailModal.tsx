...
const DetailModal: React.FC<DetailModalProps> = ({
    ...
    return (
        <BaseModal isOpen={isOpen} onClose={onClose}>
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full h-full max-h-[inherit] flex flex-col border border-davys-gray-200 overflow-hidden">
                {/* Header */}
                    ...
                    <div className="space-y-3 sm:space-y-4 w-full">
                        {data.map((item, index) => (
                            <div key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1 sm:gap-2">
                                <span className="text-xs sm:text-sm font-semibold text-davys-gray-600 flex-shrink-0 sm:w-1/3 lg:w-1/4">
                                    {item.label}:
                                </span>
                                <span className="text-sm sm:text-base text-davys-gray-100 sm:text-right flex-1 break-words min-w-0">
                                    {item.value}
                                </span>
                            </div>
                        ))}
